...
import { getOrders, getOrderById, getUserOrders, createOrder, updateOrderStatus, cancelOrder } from '../../../features/orders/controllers';
...
describe('Order Controllers', () => {
  ...
  describe('getOrderById', () => {
    ...
    it('should return 404 if order not found', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith({ error: '注文が見つかりません' }, 404);
    });
  ...
  describe('createOrder', () => {
    ...
    it('should handle product not found', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith({ error: `商品ID 999 が見つかりません` }, 404);
    });
  ...
  describe('updateOrderStatus', () => {
    it('should update an order status', async () => {
      ...
      mockDbClient.returning.mockResolvedValueOnce([{ ...mockOrder, status: 'PROCESSING' }]);

      const result = await updateOrderStatus(mockContext);

      expect(mockContext.req.param).toHaveBeenCalledWith('id');
      expect(mockContext.req.valid).toHaveBeenCalledWith('json');
      expect(mockContext.json).toHaveBeenCalledWith({ order: { ...mockOrder, status: 'PROCESSING' } });
    });
  ...
  describe('cancelOrder', () => {
    it('should cancel an order and restore product stock', async () => {
      ...
      mockDbClient.set.mockReturnThis();
      mockDbClient.where.mockReturnThis();
      mockDbClient.returning.mockResolvedValueOnce([{ ...mockOrder, status: 'CANCELLED' }]);
    ...
    it('should return 400 if order is already cancelled', async () => {
      ...
      mockDbClient.select.mockReturnThis();
      mockDbClient.from.mockReturnThis();
      mockDbClient.where.mockResolvedValueOnce([{ ...mockOrder, status: 'CANCELLED' }]);

      const result = await cancelOrder(mockContext);

      expect(mockContext.json).toHaveBeenCalledWith({ error: 'この注文は既にキャンセルされています' }, 400);
    });
    ...
    it('should return 400 if order is already shipped or delivered', async () => {
      ...
      mockDbClient.select.mockReturnThis();
      mockDbClient.from.mockReturnThis();
      mockDbClient.where.mockResolvedValueOnce([{ ...mockOrder, status: 'SHIPPED' }]);

      const result = await cancelOrder(mockContext);

      expect(mockContext.json).toHaveBeenCalledWith({ error: '発送済みまたは配達済みの注文はキャンセルできません' }, 400);
    });
