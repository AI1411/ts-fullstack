...
export const createInvoice = async (c: Context) => {
  ...
  try {
    ...
    return c.json({ invoice: invoice[0] }, 201);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
};
...
export const updateInvoice = async (c: Context) => {
  ...
  try {
    ...
    // 注文の存在確認（order_idが提供された場合）
    if (data.order_id) {
      const order = await db.select().from(ordersTable).where(eq(ordersTable.id, data.order_id));
      if (!order.length) {
        return c.json({ error: '指定された注文が見つかりません' }, 404);
      }
    }
    ...
    return c.json({ invoice: updatedInvoice[0] });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
};
