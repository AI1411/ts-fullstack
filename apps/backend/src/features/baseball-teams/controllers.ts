...
export const getBaseballTeamById = async (c: Context) => {
  ...
  try {
    const team = await db.select().from(baseballTeamsTable).where(eq(baseballTeamsTable.id, id));
...
export const createBaseballTeam = async (c: Context) => {
  ...
  try {
    ...
    if (data.abbreviation !== undefined) teamData.abbreviation = data.abbreviation;
    if (data.league !== undefined) teamData.league = data.league;
    if (data.division !== undefined) teamData.division = data.division;
    if (data.home_stadium !== undefined) teamData.home_stadium = data.home_stadium;
    if (data.city !== undefined) teamData.city = data.city;
    if (data.founded_year !== undefined) teamData.founded_year = data.founded_year;
    ...
    const team = await db.insert(baseballTeamsTable).values(teamData).returning();
...
export const updateBaseballTeam = async (c: Context) => {
  ...
  try {
    // チームの存在確認
    const existingTeam = await db.select().from(baseballTeamsTable).where(eq(baseballTeamsTable.id, id));
    ...
    if (data.home_stadium !== undefined) updateData.home_stadium = data.home_stadium;
    ...
    // チームを更新
    const updatedTeam = await db.update(baseballTeamsTable)
      .set(updateData)
