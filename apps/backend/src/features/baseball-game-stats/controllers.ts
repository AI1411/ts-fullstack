...
export const createBaseballGameStat = async (c: Context) => {
  ...
  try {
    ...
    const player = await db
      .select()
      .from(baseballPlayersTable)
      .where(eq(baseballPlayersTable.id, player_id));
...
export const updateBaseballGameStat = async (c: Context) => {
  ...
  try {
    ...
    if (data.game_date !== undefined)
      updateData.game_date = new Date(data.game_date);
