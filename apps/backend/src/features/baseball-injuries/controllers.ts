...
export const createBaseballInjury = async (c: Context) => {
  ...
  try {
    ...
    if (data.end_date !== undefined)
      injuryData.end_date = new Date(data.end_date);
    ...
    const injury = await db
      .insert(baseballPlayerInjuriesTable)
      .values(injuryData)
      .returning();
...
export const updateBaseballInjury = async (c: Context) => {
  ...
  try {
    ...
    if (data.injury_type !== undefined)
      updateData.injury_type = data.injury_type;
    ...
    // 怪我記録を更新
    const updatedInjury = await db
      .update(baseballPlayerInjuriesTable)
      .set(updateData)
      .where(eq(baseballPlayerInjuriesTable.id, id))
      .returning();
...
export const deleteBaseballInjury = async (c: Context) => {
  ...
  try {
    ...
    // 怪我記録を削除
    await db
      .delete(baseballPlayerInjuriesTable)
      .where(eq(baseballPlayerInjuriesTable.id, id));
