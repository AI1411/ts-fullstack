...
export const createCompany = async (c: Context) => {
  const { name, description, address, phone, email, website } = c.req.valid('json');
  const db = getDB(c);
...
export const updateCompany = async (c: Context) => {
  ...
  try {
    // 会社の存在確認
    const existingCompany = await db.select().from(companiesTable).where(eq(companiesTable.id, id));
