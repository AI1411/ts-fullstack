...
import { baseballPlayerMediaTable, baseballPlayersTable } from '../../db/schema';
...
export const getBaseballPlayerMediaById = async (c: Context) => {
  ...
  try {
    const mediaItem = await db.select().from(baseballPlayerMediaTable).where(eq(baseballPlayerMediaTable.id, id));
...
export const createBaseballPlayerMedia = async (c: Context) => {
  ...
  try {
    ...
    if (data.thumbnail_url !== undefined) mediaData.thumbnail_url = data.thumbnail_url;
    if (data.publication_date !== undefined) mediaData.publication_date = new Date(data.publication_date);
    if (data.source !== undefined) mediaData.source = data.source;
    if (data.is_featured !== undefined) mediaData.is_featured = data.is_featured;
    
    const mediaItem = await db.insert(baseballPlayerMediaTable).values(mediaData).returning();
...
export const updateBaseballPlayerMedia = async (c: Context) => {
  ...
  try {
    // メディアコンテンツの存在確認
    const existingMedia = await db.select().from(baseballPlayerMediaTable).where(eq(baseballPlayerMediaTable.id, id));
    ...
    if (data.publication_date !== undefined) updateData.publication_date = new Date(data.publication_date);
    ...
    // メディアコンテンツを更新
    const updatedMedia = await db.update(baseballPlayerMediaTable)
      .set(updateData)
...
export const deleteBaseballPlayerMedia = async (c: Context) => {
  ...
  try {
    ...
    // メディアコンテンツを削除
    await db.delete(baseballPlayerMediaTable).where(eq(baseballPlayerMediaTable.id, id));

    return c.json({ success: true, message: 'メディアコンテンツが削除されました' });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
};
...
export const getFeaturedBaseballPlayerMedia = async (c: Context) => {
  ...
  try {
    const featuredMedia = await db.select()
      .from(baseballPlayerMediaTable)
