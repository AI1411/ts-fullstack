...
export const baseballPlayersTable = pgTable("baseball_players", {
  ...
  on_base_percentage: numeric("on_base_percentage", { precision: 4, scale: 3 }),
  slugging_percentage: numeric("slugging_percentage", { precision: 4, scale: 3 }),
  ...
  return {
    ...
    birthDateIdx: index("idx_baseball_players_birth_date").on(table.birth_date),
    throwingHandIdx: index("idx_baseball_players_throwing_hand").on(table.throwing_hand),
    battingHandIdx: index("idx_baseball_players_batting_hand").on(table.batting_hand),
...
// 野球チームテーブル
export const baseballTeamsTable = pgTable("baseball_teams", {
  id: serial("id").primaryKey(),
  name: varchar("name", {length: 100}).notNull(),
...
// 野球試合テーブル
export const baseballGamesTable = pgTable("baseball_games", {
  id: serial("id").primaryKey(),
  date: timestamp("date").notNull(),
  home_team_id: integer("home_team_id").references(() => baseballTeamsTable.id, {onDelete: "cascade"}).notNull(),
  away_team_id: integer("away_team_id").references(() => baseballTeamsTable.id, {onDelete: "cascade"}).notNull(),
  ...
  return {
    ...
    homeTeamIdIdx: index("idx_baseball_games_home_team_id").on(table.home_team_id),
    awayTeamIdIdx: index("idx_baseball_games_away_team_id").on(table.away_team_id),
...
export const baseballPlayerInjuriesTable = pgTable("baseball_player_injuries", {
  id: serial("id").primaryKey(),
  ...
  return {
    playerIdIdx: index("idx_baseball_player_injuries_player_id").on(table.player_id),
    injuryTypeIdx: index("idx_baseball_player_injuries_injury_type").on(table.injury_type),
    statusIdx: index("idx_baseball_player_injuries_status").on(table.status),
    startDateIdx: index("idx_baseball_player_injuries_start_date").on(table.start_date),
    endDateIdx: index("idx_baseball_player_injuries_end_date").on(table.end_date),
    createdAtIdx: index("idx_baseball_player_injuries_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_player_injuries_updated_at").on(table.updated_at)
  };
});

// 野球選手のメディアコンテンツテーブル
export const baseballPlayerMediaTable = pgTable("baseball_player_media", {
  id: serial("id").primaryKey(),
  player_id: integer("player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  ...
  return {
    playerIdIdx: index("idx_baseball_player_media_player_id").on(table.player_id),
    mediaTypeIdx: index("idx_baseball_player_media_media_type").on(table.media_type),
    isFeaturedIdx: index("idx_baseball_player_media_is_featured").on(table.is_featured),
    publicationDateIdx: index("idx_baseball_player_media_publication_date").on(table.publication_date),
    createdAtIdx: index("idx_baseball_player_media_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_player_media_updated_at").on(table.updated_at)
  };
});
...
export const baseballGameStatsTable = pgTable("baseball_game_stats", {
  id: serial("id").primaryKey(),
  player_id: integer("player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  game_id: integer("game_id").references(() => baseballGamesTable.id, {onDelete: "cascade"}).notNull(),
  team_id: integer("team_id").references(() => baseballTeamsTable.id, {onDelete: "cascade"}).notNull(),
  // 打撃成績
  at_bats: integer("at_bats"),
...
// 野球選手の対戦相手別成績テーブル
export const baseballPlayerMatchupsTable = pgTable("baseball_player_matchups", {
  id: serial("id").primaryKey(),
  player_id: integer("player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  opponent_player_id: integer("opponent_player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  opponent_team_id: integer("opponent_team_id").references(() => baseballTeamsTable.id, {onDelete: "cascade"}).notNull(),
  // 打者 vs 投手の成績
  at_bats: integer("at_bats"),
  hits: integer("hits"),
  doubles: integer("doubles"),
  ...
  return {
    playerIdIdx: index("idx_baseball_player_matchups_player_id").on(table.player_id),
    opponentPlayerIdIdx: index("idx_baseball_player_matchups_opponent_player_id").on(table.opponent_player_id),
    opponentTeamIdIdx: index("idx_baseball_player_matchups_opponent_team_id").on(table.opponent_team_id),
    seasonYearIdx: index("idx_baseball_player_matchups_season_year").on(table.season_year),
    isCareerIdx: index("idx_baseball_player_matchups_is_career").on(table.is_career),
    createdAtIdx: index("idx_baseball_player_matchups_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_player_matchups_updated_at").on(table.updated_at)
  };
});

// 野球選手のシーズン成績テーブル
export const baseballPlayerSeasonsTable = pgTable("baseball_player_seasons", {
  id: serial("id").primaryKey(),
  player_id: integer("player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  team_id: integer("team_id").references(() => baseballTeamsTable.id, {onDelete: "cascade"}).notNull(),
  ...
  errors: integer("errors"),
  double_plays: integer("double_plays"),
  fielding_percentage: numeric("fielding_percentage", { precision: 4, scale: 3 }),
  ...
  return {
    playerIdIdx: index("idx_baseball_player_seasons_player_id").on(table.player_id),
    teamIdIdx: index("idx_baseball_player_seasons_team_id").on(table.team_id),
    seasonYearIdx: index("idx_baseball_player_seasons_season_year").on(table.season_year),
    createdAtIdx: index("idx_baseball_player_seasons_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_player_seasons_updated_at").on(table.updated_at)
  };
});

// 野球選手のキャリア成績テーブル
export const baseballPlayerCareersTable = pgTable("baseball_player_careers", {
  id: serial("id").primaryKey(),
  player_id: integer("player_id").references(() => baseballPlayersTable.id, {onDelete: "cascade"}).notNull(),
  // 打撃成績
  ...
  career_batting_average: numeric("career_batting_average", { precision: 4, scale: 3 }),
  career_on_base_percentage: numeric("career_on_base_percentage", { precision: 4, scale: 3 }),
  career_slugging_percentage: numeric("career_slugging_percentage", { precision: 4, scale: 3 }),
  ...
  total_innings_pitched: numeric("total_innings_pitched", { precision: 7, scale: 1 }),
  ...
  career_pitcher_war: numeric("career_pitcher_war", { precision: 5, scale: 1 }),
  // 守備成績
  total_putouts: integer("total_putouts"),
  total_assists: integer("total_assists"),
  total_errors: integer("total_errors"),
  career_fielding_percentage: numeric("career_fielding_percentage", { precision: 4, scale: 3 }),
  // 受賞歴・記録
  awards: text("awards"),
  ...
  return {
    playerIdIdx: index("idx_baseball_player_careers_player_id").on(table.player_id),
    lastUpdatedIdx: index("idx_baseball_player_careers_last_updated").on(table.last_updated),
    createdAtIdx: index("idx_baseball_player_careers_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_player_careers_updated_at").on(table.updated_at)
  };
});

// 野球データのインポート/エクスポート履歴テーブル
export const baseballDataImportExportTable = pgTable("baseball_data_import_export", {
  id: serial("id").primaryKey(),
  ...
  return {
    operationTypeIdx: index("idx_baseball_data_import_export_operation_type").on(table.operation_type),
    dataTypeIdx: index("idx_baseball_data_import_export_data_type").on(table.data_type),
    statusIdx: index("idx_baseball_data_import_export_status").on(table.status),
    executedAtIdx: index("idx_baseball_data_import_export_executed_at").on(table.executed_at),
    createdAtIdx: index("idx_baseball_data_import_export_created_at").on(table.created_at),
    updatedAtIdx: index("idx_baseball_data_import_export_updated_at").on(table.updated_at)
  };
});
