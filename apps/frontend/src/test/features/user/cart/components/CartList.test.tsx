...
vi.mock('@/features/user/cart/components/CartItem', () => ({
  default: vi.fn(({ id, name, price, quantity, image, onRemove, onUpdateQuantity }) => (
...
describe('CartList Component', () => {
  ...
  it('should render the component with items', () => {
    ...
    expect(screen.getByTestId('item-name-1').textContent).toBe('Test Product 1');
    expect(screen.getByTestId('item-price-1').textContent).toBe('1000');
    expect(screen.getByTestId('item-quantity-1').textContent).toBe('1');
    expect(screen.getByTestId('item-image-1').textContent).toBe('test-image-1.jpg');
    
    expect(screen.getByTestId('item-name-2').textContent).toBe('Test Product 2');
    expect(screen.getByTestId('item-price-2').textContent).toBe('2000');
    expect(screen.getByTestId('item-quantity-2').textContent).toBe('2');
    expect(screen.getByTestId('item-image-2').textContent).toBe('test-image-2.jpg');
  });
