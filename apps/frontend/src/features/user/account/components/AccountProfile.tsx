...
const AccountProfile: React.FC<AccountProfileProps> = ({ user }) => {
  return (
    <div className="p-6">
      <h2 className="mb-6 text-xl font-semibold text-gray-900 dark:text-white">プロフィール情報</h2>
      ...
      <div className="mb-8 flex flex-col items-center sm:flex-row sm:items-start">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">会員登録日: {user.memberSince}</p>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div>
          <h3 className="mb-3 text-lg font-medium text-gray-900 dark:text-white">基本情報</h3>
        ...
        <div>
          <h3 className="mb-3 text-lg font-medium text-gray-900 dark:text-white">住所情報</h3>
          <div className="space-y-4 rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">郵便番号</p>
              <p className="font-medium text-gray-900 dark:text-white">{user.address.postal}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">住所</p>
              <p className="font-medium text-gray-900 dark:text-white">
                {user.address.prefecture}{user.address.city}{user.address.line1}
              </p>
              <p className="font-medium text-gray-900 dark:text-white">{user.address.line2}</p>
            </div>
          </div>
          <button className="mt-3 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
            住所情報を編集
