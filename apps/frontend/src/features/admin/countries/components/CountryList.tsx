...
const CountryList = () => {
  ...
  const [expandedCountryId, setExpandedCountryId] = useState<number | null>(
    null
  );
  ...
  if (!countries || countries.length === 0)
    return <div className="text-center py-4">国がありません</div>;
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {countries.map((country) => (
            <React.Fragment key={country.id}>
              <tr
                className={expandedCountryId === country.id ? 'bg-gray-50' : ''}
              >
